...
contract ControllerFacet is Common, ReentrancyGuard, Initializable {
    ...
    function deposit(
        ...
        if (_activated == true) {
            if (msg.sender == safeOperations) {
                ...
                shares = activePool.deposit(_amount, address(this));
            ...
            console.log("Minted %s unactiveTokens to User", mintAmount);
        ...
        returns (uint redemptionAmount)
    {
        ...
        console.log("Burned %s unactiveTokens from User", _amount);
        ...
        )
    {
        ...
        if (_activated == true) {
            ...
            activeTokens = activePool.redeem(
                _shares,
                _withdrawer,
                address(this)
            );
            ...
            console.log("Burned %s activeTokens from Controller", activeTokens);
        ...
        )
    {
        ...
        console.log("Total value in Vault: %s", vaultValue);
        ...
        if (vaultValue > activeTokenContractSupply) {
            ...
            activeTokenContract.changeSupply(
                activeTokenContractSupply + holderYield
            );
